...
A web-based version of the PID Annotator tool that runs in Docker containers, specifically optimized for Raspberry Pi 5 with CasaOS. Automatically annotates P&ID (Piping & Instrumentation Diagram) PDF documents with metadata from Excel component lists.
...
- **Frontend**: React 18 with TailwindCSS (not Bootstrap)
...
├── pid_annotator_core.py       # Core processing engine (from desktop app)
├── templates/
...
The development server runs on `http://localhost:5001` by default.
