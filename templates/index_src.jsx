        ...
        <>
          <button
            ...
            className={cx(
              ...
              "border border-brand/30 text-brand/50 dark:border-brand/20 dark:text-brand/40 hover:border-brand hover:text-brand dark:hover:border-brand dark:hover:text-brand",
          ...
          {/* Tooltip - Fixed positioning to break out of container constraints */}
          {isOpen && (
            <div
              ref={tooltipRef}
    ...
    function ThemeToggle() {
      ...
      return (
        ...
        >
          <i className={cx("fa-solid", dark ? "fa-moon" : "fa-sun")}></i>
    ...
    function DeleteBrowserDataButton() {
      const handleDeleteData = () => {
        const confirmed = window.confirm(
          "This will delete all saved browser data for this app (settings, preferences, cache). This action cannot be undone. Are you sure?"
        );
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400">
                    Annotate PDF files using Excel data
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <DeleteBrowserDataButton />
                <ThemeToggle />
              </div>
            </div>
          </div>
    ...
    function SectionCard({ title, children, right }) {
      ...
      return (
        <section className="rounded-xl bg-white dark:bg-zinc-950/60 border border-zinc-200/60 dark:border-zinc-800 shadow-soft">
          <button
            type="button"
            onClick={() => setOpen((o) => !o)}
          ...
          >
            <div className="flex items-center gap-3">
              <i className={cx("fa-solid text-brand", open ? "fa-chevron-down rotate-0" : "fa-chevron-right rotate-0", "transition-transform duration-200")}></i>
    ...
    function UploadArea({ onFilesUploaded, onColumnsLoaded, uploadedFiles, setUploadedFiles, setToast }) {
      ...
      function validateAndUpload(files) {
        ...
        logToConsole(
          `Debug: hasExcelInCurrentUpload=${hasExcelInCurrentUpload}, hasExcelAlreadyUploaded=${hasExcelAlreadyUploaded}, uploadedFiles.excel.length=${uploadedFiles.excel.length}`
        );
        ...
        if (hasExcelInCurrentUpload && hasExcelAlreadyUploaded) {
          setStatusMsg(
            "error",
            "Excel file already uploaded. Please upload only PDF files now."
          );
        ...
        Promise.all(uploadPromises).then((results) => {
          ...
          if (successful === total) {
            ...
            setStatusMsg(
              "warning",
              `${successful}/${total} files uploaded`
            );
            ...
            .then((data) => {
              if (data.success) {
                ...
                setUploadedFiles((prev) => {
                  const next = { ...prev };
                  next[type] = [...prev[type], { name: file.name, filename: data.filename }];
                  return next;
                });
      ...
      return (
        <div>
          <div
            ...
            className={cx(
              "group cursor-pointer rounded-2xl border-2 border-dashed p-8 sm:p-10 text-center transition-colors duration-300",
              "bg-white dark:bg-zinc-950/40 border-zinc-300/60 dark:border-zinc-700",
              "hover:border-brand/60 hover:bg-brand/5",
              dragOver && "border-brand bg-brand/10"
            )}
          >
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-xl bg-brand/10 text-brand mb-3">
              <i className="fa-solid fa-file-arrow-up text-2xl"></i>
            </div>
            <div className="text-lg font-semibold">Click or drag files here.</div>
          ...
          {status && (
            ...
            >
              {status.type === "processing" && <i className="fa-solid fa-spinner animate-spin"></i>}
              {status.type === "success" && <i className="fa-solid fa-check"></i>}
              {status.type === "warning" && <i className="fa-solid fa-triangle-exclamation"></i>}
              {status.type === "error" && <i className="fa-solid fa-xmark"></i>}
    ...
    function FilesList({ uploadedFiles }) {
      const hasFiles =
        uploadedFiles.excel.length > 0 || uploadedFiles.pdf.length > 0;
    ...
    function FileWorkspace({ workspaceFiles, selectedPdfs, selectedExcel, onSelectionChange, onExcelSelectionChange, onDeleteFile, onUploadFiles, onColumnsLoaded, setToast, onRefresh, loadingColumns, setLoadingColumns }) {
          ...
          fetch(type === "pdf" ? "/upload_pdf" : "/upload_excel", {
            ...
            .then((data) => {
              if (data.success) {
                ...
                setToast({ type: "success", text: `${file.name} uploaded successfully` });
                resolve({ success: true, filename: data.filename, type: type });
              } else {
                setToast({ type: "error", text: data.message });
      ...
      function handleDeleteFile(filename) {
        ...
        fetch("/delete_file", {
          ...
          .then((data) => {
            if (data.success) {
              setToast({ type: "success", text: "File deleted" });
      ...
      function togglePdfSelection(filename) {
        const newSelection = selectedPdfs.includes(filename)
          ? selectedPdfs.filter(f => f !== filename)
      ...
      function toggleExcelSelection(filename) {
        ...
        fetch("/select_excel", {
          ...
          .catch((err) => {
            ...
            setToast({ type: "error", text: "Failed to switch Excel file" });
          })
          .finally(() => {
            setLoadingColumns(false);
          });
      ...
      return (
        <div className="space-y-4">
          ...
          <div
            ...
            className={cx(
              "cursor-pointer rounded-xl border-2 border-dashed p-6 text-center transition-colors duration-300",
              "bg-white dark:bg-zinc-950/40 border-zinc-300/60 dark:border-zinc-700",
              "hover:border-brand/60 hover:bg-brand/5",
              dragOver && "border-brand bg-brand/10"
            )}
          >
            <div className="flex items-center justify-center gap-3">
              <i className="fa-solid fa-cloud-arrow-up text-2xl text-brand"></i>
              <div className="text-sm font-medium">Drop files here or click to upload</div>
              <div className="text-xs text-zinc-500 dark:text-zinc-400">(PDF, Excel, CSV)</div>
            </div>
            <input
              ref={inputRef}
              type="file"
          ...
          {totalFiles > 0 && (
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">
                Workspace ({totalFiles} file{totalFiles !== 1 ? 's' : ''})
                {selectedCount > 0 && (
              ...
              <div className="flex gap-2">
                {workspaceFiles.pdfs.length > 0 && (
                  <>
                    {selectedPdfs.length === workspaceFiles.pdfs.length ? (
                      <button
                  ...
                  <div
                    ...
                    className={cx(
                      ...
                      isSelected
                        ...
                        : "border-zinc-200/60 dark:border-zinc-800 bg-white dark:bg-zinc-950/60"
                    )}
                  >
                    <input
                      type="checkbox"
                    ...
                    />
                    <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-500">
                      <i className={cx("fa-solid", file.type === 'csv' ? 'fa-file-csv' : 'fa-file-excel')}></i>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium truncate">{file.name}</div>
                      <div className="text-xs text-zinc-500 dark:text-zinc-400 flex items-center gap-2">
                        <span>{formatFileSize(file.size)}</span>
                        <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-medium">TAG SOURCE</span>
                        {isSelected && (
                          <span className="px-2 py-0.5 rounded bg-brand/10 text-brand font-medium">SELECTED</span>
                        )}
                      </div>
                    </div>
                    <button
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-zinc-500 dark:text-zinc-400 flex items-center gap-2">
                        <span>{formatFileSize(file.size)}</span>
                        {file.isAnnotated && (
                          <span className="px-2 py-0.5 rounded bg-brand/10 text-brand font-medium">ANNOTATED</span>
                        )}
                        {file.isTest && (
                          <span className="px-2 py-0.5 rounded bg-amber-500/10 text-amber-600 dark:text-amber-400 font-medium">TEST</span>
                        )}
                      </div>
                    </div>
                    <button
          ...
          {totalFiles === 0 && (
            <div className="text-center py-8 text-zinc-500 dark:text-zinc-400">
              ...
              <div className="text-sm">No files in workspace. Upload files to get started.</div>
            </div>
          )}
        </div>
      );
              ...
              <button
                ...
                className={cx(
                  ...
                  current === t.key
                    ...
                    : "bg-zinc-200/60 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-300/60 dark:hover:bg-zinc-700"
                )}
                role="tab"
                aria-selected={current === t.key}
              >
                {t.icon && <i className={cx("mr-2", t.icon)}></i>}
                {t.label}
              </button>
            ))}
          </div>
      ...
      return (
        ...
        >
          <div
            ...
            onClick={() => {}}
          />
          <div className="relative w-full max-w-md rounded-2xl border border-zinc-200/60 dark:border-zinc-800 bg-white dark:bg-zinc-950/90 shadow-soft">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200/60 dark:border-zinc-800">
              <div className="flex items-center gap-2">
                <i className={cx("fa-solid text-brand", isTestRun ? "fa-bolt" : "fa-cogs")}></i>
            ...
            {!error && (
              <div className="px-5 py-4">
                <div className="h-3 w-full overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-800 mb-3">
                  <div
                ...
                {!!fileCount && (
                  <div className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
                    Processing {fileCount} PDF file(s)
                  </div>
                )}
                ...
                {complete && (
                  <div className="mt-5 space-y-3">
                    <div className="flex items-center gap-2 text-emerald-500">
                      ...
                      <div className="font-semibold">
                        {isTestRun
                          ...
                          : (annotateExcelEnabled
                              ? (fileCount > 1
                                  ? `Your ${fileCount} annotated PDF and Excel files are ready.`
                                  : "Your annotated PDF and Excel files are ready.")
                              : (fileCount > 1
                                  ? `Your ${fileCount} annotated PDF files are ready.`
                    ...
                    {isTestRun && (
                      <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200/60 dark:border-blue-800 px-3 py-2">
                        <div className="flex items-start gap-2">
                          <i className="fa-solid fa-info-circle text-blue-600 dark:text-blue-400 mt-0.5"></i>
                          <div className="text-xs text-blue-800 dark:text-blue-200">
                            Upload data has been preserved. You can now review the test output and run the full processing when ready.
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid gap-2">
                      ...
                      >
                        ...
                        {isTestRun
                          ? "Download Test PDF(s)"
                          : (fileCount > 1 ? `Download ${fileCount} PDF(s)` : "Download PDF")
                        }
                      </button>
            ...
            {error && (
              <div className="px-5 py-6">
                <div className="flex flex-col items-center text-center">
                  <i className="fa-solid fa-triangle-exclamation text-danger text-4xl mb-3"></i>
                  <div className="text-xl font-semibold text-danger mb-2">
                    Processing Failed
                  </div>
    ...
    function SaveProfileModal({ open, onClose, onSave }) {
      ...
      return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-zinc-200/60 dark:border-zinc-800 bg-white dark:bg-zinc-950/90 shadow-soft">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200/60 dark:border-zinc-800">
              <h3 className="font-semibold">Save Configuration Profile</h3>
              <button
                onClick={onClose}
                className="rounded-lg p-2 hover:bg-zinc-200/60 dark:hover:bg-zinc-800"
                aria-label="Close"
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div>
                <label htmlFor="profile-description" className="block text-sm font-medium mb-1">
    ...
    function ImportProfileModal({ open, onClose, onImport }) {
      ...
      return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-zinc-200/60 dark:border-zinc-800 bg-white dark:bg-zinc-950/90 shadow-soft">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200/60 dark:border-zinc-800">
              <h3 className="font-semibold">Import Configuration Profile</h3>
              <button
                onClick={onClose}
                className="rounded-lg p-2 hover:bg-zinc-200/60 dark:hover:bg-zinc-800"
                aria-label="Close"
            ...
            <div className="px-5 py-4 space-y-4">
              <div className="text-sm text-zinc-600 dark:text-zinc-400">
                Select a JSON profile file to import. The profile must match the expected format.
              </div>
              <div
              ...
              >
                ...
                <div className="text-lg font-semibold">Click to select JSON file</div>
    ...
    function FAQModal({ open, onClose }) {
        ...
        'getting-started': {
          ...
          questions: [
            ...
            },
            {
              ...
              a: (
                <div>
                  <p className="mb-2">Yes! The source code is available on GitHub for free and you can run it locally:</p>
                  <p className="mb-2">
                    <a href="https://github.com/rudikdk/Annotator" target="_blank" rel="noopener noreferrer" className="text-brand hover:text-brand-hover font-semibold flex items-center gap-1">
                      <i className="fa-brands fa-github"></i>
                      github.com/rudikdk/Annotator
                    </a>
                  </p>
                  <p className="text-sm">You can clone the repository and run it locally with Docker or Python. Perfect for local deployments, private use, or customization. Full setup instructions are available in the repository README.</p>
                </div>
              )
            }
          ]
          ...
          questions: [
            {
              ...
              a: (
                <div>
                  <p className="mb-2">This application uses the following open-source Python libraries:</p>
                  <ul className="list-disc list-inside space-y-1 text-xs">
                    <li>Flask (BSD-3-Clause) - Web framework</li>
                    <li>Flask-SocketIO (MIT) - Real-time communication</li>
                    <li>pandas (BSD-3-Clause) - Data processing</li>
                    <li>PyMuPDF (AGPL-3.0) - PDF manipulation</li>
                    <li>openpyxl (MIT) - Excel file handling</li>
                    <li>Werkzeug (BSD-3-Clause) - WSGI utilities</li>
                    <li>python-socketio (MIT) - WebSocket support</li>
                    <li>eventlet (MIT) - Concurrent networking</li>
                    <li>gunicorn (MIT) - Production server</li>
                    <li>reportlab (BSD) - PDF generation</li>
                    <li>PyPDF2 (BSD-3-Clause) - PDF merging</li>
                  </ul>
                </div>
              )
            },
      ...
      return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose}/>
          <div className="relative w-full max-w-4xl rounded-2xl border border-zinc-200/60 dark:border-zinc-800 bg-white dark:bg-zinc-950/90 shadow-soft flex flex-col max-h-[85vh]">
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200/60 dark:border-zinc-800">
              <div>
                <h3 className="font-semibold text-lg">FAQ – PID Annotator</h3>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">Frequently Asked Questions & Help</p>
              </div>
              <button
                onClick={onClose}
                className="rounded-lg p-2 hover:bg-zinc-200/60 dark:hover:bg-zinc-800"
            ...
            {/* Search bar */}
            <div className="px-5 py-3 border-b border-zinc-200/60 dark:border-zinc-800">
              <div className="relative">
            ...
            <div className="flex flex-1 overflow-hidden">
              {/* Category sidebar */}
              <div className="w-56 border-r border-zinc-200/60 dark:border-zinc-800 overflow-y-auto">
                <div className="p-2 space-y-1">
                  {Object.entries(filteredCategories).map(([key, category]) => (
                    ...
                    >
                      <i className={cx('fa-solid', category.icon, 'w-4')}></i>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4">
                {Object.keys(filteredCategories).length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {filteredCategories[activeCategory]?.questions.map((item, idx) => (
                      <div key={idx} className="space-y-2">
                        <h4 className="font-semibold text-brand flex items-start gap-2">
                          <i className="fa-solid fa-circle-question text-sm mt-0.5"></i>
                          {item.q}
                        </h4>
                        <div className="text-sm text-zinc-700 dark:text-zinc-300 pl-6">
            ...
            {/* Footer */}
            <div className="px-5 py-3 border-t border-zinc-200/60 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
              <div className="text-xs text-zinc-500 dark:text-zinc-400">
                <p><strong>Created by:</strong> Rudi S. Kærgaard — <strong>Email:</strong> rudikdk@gmail.com</p>
              </div>
            </div>
          </div>
        </div>
    ...
    function App() {
      ...
      const [uploadedFiles, setUploadedFiles] = useState({ excel: [], pdf: [] });

      // Workspace state
      const [workspaceFiles, setWorkspaceFiles] = useState({ excel: [], pdfs: [] });
      ...
      function handlePreviewTagMatching() {
        if (!selectedPdfs || selectedPdfs.length === 0) {
          setToast({ type: "error", text: "Please select a PDF file first" });
          return;
        }
          ...
          .then((data) => {
            ...
            if (data.success) {
              ...
              if (data.total_matched === 0) {
                setToast({ type: "warning", text: "No tags matched the current configuration" });
              }
            } else {
              setToast({ type: "error", text: data.message || "Failed to preview tag matching" });
              logToConsole(`Error previewing tag matching: ${data.message}`);
            }
          })
          .catch((err) => {
            setTagMatchingPreviewLoading(false);
            setToast({ type: "error", text: "Error fetching tag matching preview" });
      ...
      function handleAddColorRule() {
        ...
        if (newRuleType === 'header_column' && !newRuleColumn) {
          setToast({ type: "error", text: "Please select a column for header-based coloring" });
          return;
        }
      ...
      function handleRemoveColorRule(ruleId) {
        ...
        setToast({ type: "success", text: "Color rule removed" });
      }

      function handleClearAllColorRules() {
        setColorRules([]);
        setToast({ type: "success", text: "All color rules cleared" });
      }
      ...
      function openPreviewModal() {
        if (selectedPdfs.length === 0 || !selectedExcel) {
          setToast({ type: "warning", text: "Please upload PDF and Excel files first" });
          return;
        }
        ...
        fetch('/generate_full_preview', {
          ...
          .then(data => {
            ...
            if (data.success) {
              ...
              setToast({ type: "success", text: `Preview ready! Page ${data.page_number} processed successfully.` });
            } else {
              setToast({ type: "error", text: data.message || 'Failed to load preview' });
            }
          })
          .catch(err => {
            setPreviewModalLoading(false);
      ...
      function handleModalPageChange(direction) {
        ...
        const newPage = direction === 'next'
          ? Math.min(previewModalPage + 1, previewModalData.total_pages)
          : Math.max(previewModalPage - 1, 1);
      ...
      useEffect(() => {
        ...
        loadingTask.promise.then(pdf => {
          ...
          if (isMounted) {
            ...
            setToast({ type: 'error', text: 'Failed to load PDF preview' });
          }
        });
      ...
      useEffect(() => {
        ...
        pdfDocument.getPage(1).then(page => {
          const viewport = page.getViewport({ scale: previewModalZoom });
      ...
      function startProcessing(isTest) {
        ...
        if (selectedPdfs.length === 0) {
          setToast({ type: "error", text: "Please select at least one PDF file to process" });
          return;
        }

        if (!selectedExcel) {
          setToast({ type: "error", text: "Please select an Excel/CSV file from the workspace" });
          return;
        }
        ...
        const data = {
          ...
          is_test: isTest,
          selected_pdfs: selectedPdfs,  // Send selected PDF filenames
          selected_excel: selectedExcel,  // Send selected Excel filename
          tag_matching_config: {
            preset: tagMatchingPreset,
            min_parts: tagMatchingMinParts,
            max_parts: tagMatchingMaxParts,
          ...
          tag_filters: tagFilters,  // Send tag filters
          filter_logic: filterLogic,  // Send filter logic (AND/OR)
          color_rules: colorRules,  // Send color rules
          default_highlight_color: defaultHighlightColor,  // Send default highlight color
          enable_default_color: enableDefaultColor,  // Send enable default color flag
          excel_constraint_mode: excelConstraintMode,  // Send excel constraint mode
      ...
      function loadFromLocalStorage() {
        try {
          ...
          if (saved) {
            ...
            setToast({ type: "success", text: "Settings restored from browser" });
            return true;
          }
        } catch (err) {
      ...
      function loadSelectedProfile() {
        if (!selectedProfile) {
          setToast({ type: "warning", text: "No profile selected" });
          return;
        }
          ...
          fetch(`/api/profiles/${filename}`)
            ...
            .then((data) => {
              if (data.success) {
                ...
                setToast({ type: "success", text: `Server profile "${data.profile.name}" loaded` });
      ...
      function exportCurrentProfile() {
        ...
        setToast({ type: "success", text: "Profile exported successfully" });
        ...
        reader.onload = (e) => {
          try {
            ...
            if (!profileData.settings) {
              setToast({ type: "error", text: "Invalid profile file: missing settings" });
              return;
            }
            ...
            setToast({ type: "success", text: `Profile "${profileData.name || 'Imported'}" loaded` });
            logToConsole(`Imported profile: ${profileData.name || 'unknown'}`);
            setShowImportProfileModal(false);
          } catch (err) {
            setToast({ type: "error", text: `Error parsing profile: ${err.message}` });
            logToConsole(`Error importing profile: ${err.message}`);
          }
        };

        reader.onerror = () => {
          setToast({ type: "error", text: "Error reading file" });
        };
            ...
            if (data.success) {
              ...
              if (hasExcelFiles && (!selectedExcel || !selectedExcelStillExists)) {
                ...
                fetch("/select_excel", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ excel_file: firstExcel, header_row: 6 })
                })
                  .then((r) => r.json())
                  .then((colData) => {
                    if (colData.success) {
      ...
      function addOutputsToWorkspace(outputFiles) {
        ...
        fetch("/add_outputs_to_workspace", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ output_files: outputFiles })
        })
          .then((r) => r.json())
          .then((data) => {
            if (data.success) {
      ...
      useEffect(() => {
        ...
        headerRow, tagColumn, commentColumns, highlightColumn, highlightColor,
        annotateExcel, watermarkEnabled, watermarkAttributes, watermarkTextColor,
        watermarkBackgroundEnabled, tagMatchingPreset, tagMatchingMinParts,
        tagMatchingMaxParts, tagMatchingSeparators, tagMatchingMinPartLength,
        tagMatchingMaxPartLength, tagMatchingAllowPartial, tagMatchingCustomRegex,
        colorRules, defaultHighlightColor, enableDefaultColor,
        excelConstraintMode, excelConstraintLogic,
        tagFilters, filterLogic
      ]);
      ...
      useEffect(() => {
        ...
        function onProgress(data) {
          logToConsole(
            `[DEBUG] Received progress update: task_id=${data.task_id}, progress=${data.progress}, status=${data.status}`
          );
        ...
        fetch(`/download?type=${fileType}`)
          ...
          .then((data) => {
            ...
            if (data.success && Array.isArray(data.files)) {
              ...
              setToast({ type: "error", text: data.message || "Download error" });
            }
          })
          .catch((err) => {
            logToConsole(`Download error: ${err.message}`);
            setToast({ type: "error", text: `Download failed: ${err.message}` });
          });
      }
        ...
        function next() {
          if (i < urls.length) {
            ...
            if (fileType !== 'report') {
              ...
              setToast({ type: "success", text: "Report downloaded successfully!" });
            }
          }
        }
        next();
        ...
        fetch("/cleanup_after_download", {
          ...
          .then((data) => {
            if (data.success) {
              ...
              setToast({ type: "error", text: `Cleanup failed: ${data.message}` });
            }
          })
          .catch((err) => {
            logToConsole(`Cleanup error: ${err.message}`);
            setToast({ type: "error", text: `Cleanup failed: ${err.message}` });
          });
      }
      ...
      function handleClearSession() {
        ...
        const confirmed = window.confirm(
          "This will delete all uploaded files and reset the session. Are you sure you want to continue?"
        );
      ...
      return (
        <div className="flex min-h-screen flex-col">
          ...
          <main className="mx-auto w-full max-w-6xl flex-1 px-4 sm:px-6 py-6 sm:py-8 space-y-6">
            ...
            <SectionCard 
              title={
                <span className="flex items-center">
                  File Workspace
                  <HelpIcon content={HELP_CONTENT.fileWorkspace} />
                </span>
              }>
              <FileWorkspace
            ...
            {/* Configuration */}
            <SectionCard title="Options & Configurations">
              <div className="space-y-6">
                {/* Basic Configuration Section */}
                <div>
                  <h3 className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 mb-3">Configuration</h3>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      <label htmlFor="header-row" className="block text-sm font-medium mb-1 flex items-center">
                        <span className="flex items-center">
                          Header Row
                          <HelpIcon content={HELP_CONTENT.headerRow} />
                        </span>
                        {loadingColumns && (
                    ...
                    <div>
                      <label htmlFor="tag-column" className="block text-sm font-medium mb-1 flex items-center">
                        <span className="flex items-center">
                          Tag Column
                          <HelpIcon content={HELP_CONTENT.tagColumn} />
                        </span>
                      </label>
                      <select
                        id="tag-column"
                      ...
                      >
                        {excelColumns.length === 0 && (
                          <option value="">Select Excel file first</option>
                        )}
                        {excelColumns.map((c) => (
                ...
                <div>
                  <h3 className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 mb-3 flex items-center">
                    <span className="flex items-center">
                      Comment Columns
                      <HelpIcon content={HELP_CONTENT.commentColumns} />
                    </span>
                  </h3>
                  <div className="mb-3 flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() =>
                        setCommentColumns(excelColumns.filter((c) => c !== tagColumn))
                      }
                  ...
                  <div className="grid gap-2" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))" }}>
                    {excelColumns.length === 0 && (
                      <div className="text-sm text-zinc-500 dark:text-zinc-400">
                        Load Excel file to see available columns
                      </div>
                    )}
                    {excelColumns
                      .filter((c) => c !== tagColumn)
                      .map((c) => {
                          ...
                          <label
                            ...
                            className={cx(
                              ...
                              checked
                                ...
                                : "border-zinc-200/60 dark:border-zinc-800 bg-transparent hover:bg-zinc-200/40 dark:hover:bg-zinc-800/60"
                            )}
                          >
                            <input
                              type="checkbox"
                ...
                {/* Excel Annotation Section */}
                <div>
                  <h3 className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 mb-3 flex items-center">
                    <span className="flex items-center">
                      Excel Annotation
                      <HelpIcon content={HELP_CONTENT.excelAnnotation} />
                    </span>
                  </h3>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="sm:col-span-2">
                      <label className={cx(
                        ...
                        !annotateExcelEnabled && "opacity-50 cursor-not-allowed"
                      )}>
                        <button
                          type="button"
                          role="switch"
                          ...
                          className={cx(
                            ...
                            annotateExcel && annotateExcelEnabled ? "bg-brand" : "bg-zinc-400/60 dark:bg-zinc-700",
                            !annotateExcelEnabled && "cursor-not-allowed opacity-50"
                          )}
                        >
                          <span
                            className={cx(
                              "block h-5 w-5 rounded-full bg-white dark:bg-zinc-200 translate-x-0.5 transition-transform",
                              annotateExcel && "translate-x-[22px]"
                            )}
                          />
                        </button>
                        <span className="text-sm font-medium">Annotate Excel file</span>
                      </label>
                      <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                        {annotateExcelEnabled ? (
                          "Highlight rows in light green where tags are found in the PDF"
                        ) : (
                          <span className="text-orange-500 dark:text-orange-400">
                            <i className="fa-solid fa-exclamation-triangle mr-1"></i>
                            Only .xlsx files support Excel annotation. Convert your .xls file to .xlsx format.
                          </span>
                        )}
                      </div>
                    </div>
                ...
                <div>
                  <h3 className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 mb-3 flex items-center">
                    <span className="flex items-center">
                      Watermark
                      <HelpIcon content={HELP_CONTENT.watermark} />
                    </span>
                  </h3>
                  <div className="space-y-4">
                    <div>
                      <label className="flex items-center gap-3">
                        <button
                          ...
                          className={cx(
                            "h-6 w-11 rounded-full transition-colors",
                            watermarkEnabled ? "bg-brand" : "bg-zinc-400/60 dark:bg-zinc-700"
                          )}
                        >
                          <span
                            className={cx(
                              "block h-5 w-5 rounded-full bg-white dark:bg-zinc-200 translate-x-0.5 transition-transform",
                              watermarkEnabled && "translate-x-[22px]"
                            )}
                          />
                        </button>
                      ...
                      <>
                        ...
                        <div>
                          <label className="flex items-center gap-2 cursor-pointer">
                            ...
                            />
                            <span className="text-sm font-medium">White background behind watermark</span>
                          </label>
                          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1 ml-6">
                            Improves readability when watermark overlaps graphics
                          </p>
                        </div>
            ...
            <SectionCard title="Advanced Options">
              <div className="space-y-4">
                {/* Configuration Profiles Sub-Card */}
                {(() => {
                  const [profilesExpanded, setProfilesExpanded] = useState(true);

                  return (
                    <div className="rounded-lg border border-zinc-200/60 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 ml-2">
                      <button
                        type="button"
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <i className={cx(
                            "fa-solid text-sm text-brand",
                            profilesExpanded ? "fa-chevron-down" : "fa-chevron-right",
                            "transition-transform duration-200"
                          )}></i>
                          <h3 className="text-sm font-semibold flex items-center">
                          <span className="flex items-center">
                            Configuration Profiles
                            <HelpIcon content={HELP_CONTENT.configurationProfiles} />
                          </span>
                        </h3>
                        </div>
                        {activeProfileName && (
                      ...
                      <div className={cx(
                        ...
                        profilesExpanded ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
                      )}>
                        <div className="overflow-hidden">
                          <div className="px-4 pb-3 space-y-3">
                            {/* Profile Selector - Load from Server */}
                            {serverProfiles.length > 0 && (
                              <div>
                                <label className="block text-sm font-medium mb-1.5">Load Profile from Server</label>
                                <div className="flex gap-2">
                                  <select
                                  ...
                                  >
                                    ...
                                    {serverProfiles.map(p => (
                                      <option key={`server:${p.filename}`} value={`server:${p.filename}`}>
                                        {p.name}
                                      </option>
                                    ))}
                                  </select>
                            ...
                            {/* Info Text */}
                            <div className="text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-100/50 dark:bg-zinc-900/50 rounded px-3 py-2 border border-zinc-200/40 dark:border-zinc-800/60">
                              <i className="fa-solid fa-circle-info mr-1.5"></i>
                              All settings auto-save to your browser, including color rules and tag filters. Export profiles as JSON to share or back up your configuration.
                            </div>

                            {/* Current Settings Preview (always visible) */}
                            {currentSettingsPreview && (
                              <div className="mt-3 rounded-lg border border-success/30 bg-success/5 p-3 space-y-2">
                                {/* Header with name and complexity badge */}
                                <div className="flex items-start justify-between pb-2 border-b border-success/20">
                                  <div className="flex-1">
                                    <h4 className="text-sm font-semibold">{currentSettingsPreview.name}</h4>
                                    {currentSettingsPreview.description && (
                                      <p className="text-xs text-zinc-600 dark:text-zinc-400 mt-1">{currentSettingsPreview.description}</p>
                                    )}
                                  </div>
                                  <span className={cx(
                                    "inline-block text-xs font-medium px-2 py-1 rounded ml-2 whitespace-nowrap",
                                    currentSettingsPreview.complexity === 'Comprehensive' && 'bg-danger/20 text-danger',
                                    currentSettingsPreview.complexity === 'Moderate' && 'bg-warning/20 text-warning',
                                    currentSettingsPreview.complexity === 'Minimal' && 'bg-success/20 text-success'
                                  )}>
                                    {currentSettingsPreview.complexity}
                                  </span>
                                </div>

                                {/* Settings Details Grid */}
                                {currentSettingsPreview.settings && (
                                  <div className="grid grid-cols-2 gap-2 text-xs">
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Header Row:</span>
                                      <span className="font-medium">{currentSettingsPreview.settings.headerRow}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Tag Column:</span>
                                      <span className="font-medium">{currentSettingsPreview.settings.tagColumn}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Highlight:</span>
                                      <span className="flex items-center gap-1">
                                        <span className="w-4 h-4 rounded border border-zinc-300 dark:border-zinc-600" style={{backgroundColor: currentSettingsPreview.settings.highlightColor}}></span>
                                        <span className="font-medium">{currentSettingsPreview.settings.highlightColumn}</span>
                                      </span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Comments:</span>
                                      <span className="font-medium">{currentSettingsPreview.settings.commentColumns}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Watermark:</span>
                                      <span className={cx(
                                        "font-medium px-1.5 py-0.5 rounded text-xs",
                                        currentSettingsPreview.settings.watermark === 'Enabled' ? 'bg-success/20 text-success' : 'bg-zinc-200/50 dark:bg-zinc-800/50 text-zinc-600 dark:text-zinc-400'
                                      )}>
                                        {currentSettingsPreview.settings.watermark}
                                      </span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Excel:</span>
                                      <span className={cx(
                                        "font-medium px-1.5 py-0.5 rounded text-xs",
                                        currentSettingsPreview.settings.annotateExcel === 'Enabled' ? 'bg-success/20 text-success' : 'bg-zinc-200/50 dark:bg-zinc-800/50 text-zinc-600 dark:text-zinc-400'
                                      )}>
                                        {currentSettingsPreview.settings.annotateExcel}
                                      </span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Tag Matching:</span>
                                      <span className="font-medium">{currentSettingsPreview.settings.tagMatchingPreset}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Color Rules:</span>
                                      <span className="font-medium">{currentSettingsPreview.settings.colorRules}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Tag Filters:</span>
                                      <span className="font-medium">{currentSettingsPreview.settings.tagFilters}</span>
                                    </div>
                                  </div>
                                )}
                              </div>
                            ...
                            {profilePreview && (
                              <div className="mt-3 rounded-lg border border-brand/30 bg-brand/5 p-3 space-y-2">
                                {/* Header with name and complexity badge */}
                                <div className="flex items-start justify-between pb-2 border-b border-brand/20">
                                  <div className="flex-1">
                                    <h4 className="text-sm font-semibold">{profilePreview.name}</h4>
                                    {profilePreview.description && (
                                      <p className="text-xs text-zinc-600 dark:text-zinc-400 mt-1">{profilePreview.description}</p>
                                    )}
                                  </div>
                                  <span className={cx(
                                    "inline-block text-xs font-medium px-2 py-1 rounded ml-2 whitespace-nowrap",
                                    profilePreview.complexity === 'Comprehensive' && 'bg-danger/20 text-danger',
                                    profilePreview.complexity === 'Moderate' && 'bg-warning/20 text-warning',
                                    profilePreview.complexity === 'Minimal' && 'bg-success/20 text-success'
                                  )}>
                                    {profilePreview.complexity}
                                  </span>
                                </div>

                                {/* Settings Details Grid */}
                                {profilePreview.settings && (
                                  <div className="grid grid-cols-2 gap-2 text-xs">
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Header Row:</span>
                                      <span className="font-medium">{profilePreview.settings.headerRow}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Tag Column:</span>
                                      <span className="font-medium">{profilePreview.settings.tagColumn}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Highlight:</span>
                                      <span className="flex items-center gap-1">
                                        <span className="w-4 h-4 rounded border border-zinc-300 dark:border-zinc-600" style={{backgroundColor: profilePreview.settings.highlightColor}}></span>
                                        <span className="font-medium">{profilePreview.settings.highlightColumn}</span>
                                      </span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Comments:</span>
                                      <span className="font-medium">{profilePreview.settings.commentColumns}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Watermark:</span>
                                      <span className={cx(
                                        "font-medium px-1.5 py-0.5 rounded text-xs",
                                        profilePreview.settings.watermark === 'Enabled' ? 'bg-success/20 text-success' : 'bg-zinc-200/50 dark:bg-zinc-800/50 text-zinc-600 dark:text-zinc-400'
                                      )}>
                                        {profilePreview.settings.watermark}
                                      </span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Excel:</span>
                                      <span className={cx(
                                        "font-medium px-1.5 py-0.5 rounded text-xs",
                                        profilePreview.settings.annotateExcel === 'Enabled' ? 'bg-success/20 text-success' : 'bg-zinc-200/50 dark:bg-zinc-800/50 text-zinc-600 dark:text-zinc-400'
                                      )}>
                                        {profilePreview.settings.annotateExcel}
                                      </span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Tag Matching:</span>
                                      <span className="font-medium">{profilePreview.settings.tagMatchingPreset}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Color Rules:</span>
                                      <span className="font-medium">{profilePreview.settings.colorRules || 'None'}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-600 dark:text-zinc-400 min-w-fit">Tag Filters:</span>
                                      <span className="font-medium">{profilePreview.settings.tagFilters || 'None'}</span>
                                    </div>
                                  </div>
                                )}
                              </div>
                ...
                {(() => {
                  ...
                  const formatFilterDisplay = (filter) => {
                    ...
                    const actionClass = filter.action === 'include'
                    ...
                    <div className="rounded-lg border border-zinc-200/60 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 ml-2">
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <i className={cx(
                            "fa-solid text-sm text-brand",
                            tagFiltersExpanded ? "fa-chevron-down" : "fa-chevron-right",
                            "transition-transform duration-200"
                          )}></i>
                          <h3 className="text-sm font-semibold flex items-center">
                          <span className="flex items-center">
                            Tag Filters
                            <HelpIcon content={HELP_CONTENT.tagFilters} />
                          </span>
                        </h3>
                        </div>
                        {tagFilters.length > 0 && (
                          <span className="text-xs px-2 py-1 rounded bg-brand/10 text-brand font-medium">
                            {tagFilters.length} {tagFilters.length === 1 ? 'filter' : 'filters'}
                          </span>
                        )}
                      </button>
                      ...
                      <div className={cx(
                        "grid transition-all duration-300",
                        tagFiltersExpanded ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
                      )}>
                        <div className="overflow-hidden">
                          <div className="px-4 pb-3 space-y-3">

                            {/* Filter Logic Selector Section */}
                            <div className="space-y-2">
                              <h4 className="text-sm font-semibold">Filter Logic</h4>
                              ...
                              <div className="text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-100/50 dark:bg-zinc-900/50 rounded px-3 py-2 border border-zinc-200/40 dark:border-zinc-800/60">
                                <i className="fa-solid fa-circle-info mr-1.5"></i>
                                {filterLogic === 'AND'
                                  ? 'Tags must match ALL filters to be processed'
                                  : 'Tags matching ANY filter will be processed'}
                              </div>
                            </div>
                            ...
                            {/* Filter Builder Sub-Section */}
                            {filterBuilderOpen && (
                              <div className="space-y-3 pt-2 border-t border-zinc-200/60 dark:border-zinc-800">
                                <div className="flex items-center justify-between">
                                  <h4 className="text-sm font-semibold">Filter Builder</h4>
                                  <button
                                ...
                                <div>
                                  ...
                                  <div className="flex gap-3">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                      ...
                                      />
                                      <span className="text-sm text-success font-medium">INCLUDE</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                      <input
                                        type="radio"
                                      ...
                                      />
                                      <span className="text-sm text-danger font-medium">EXCLUDE</span>
                                    </label>
                                  </div>
                                </div>
                                  ...
                                  <div className="flex gap-3">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                      <input
                                        ...
                                        className="accent-brand"
                                      />
                                      <span className="text-sm">Tag Part</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                      <input
                                        type="radio"
                                        ...
                                        className="accent-brand"
                                      />
                                      <span className="text-sm">Excel Column</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                      <input
                                        type="radio"
                                ...
                                {/* Tag Part Configuration */}
                                {newFilterType === 'tag_part' && (
                                  <div className="space-y-2">
                                    <label className="block text-sm font-medium">Tag Part</label>
                                    <select
                                      value={newFilterPart}
                                    ...
                                    >
                                      <option value={1}>Part A (1st part)</option>
                                      <option value={2}>Part B (2nd part)</option>
                                      <option value={3}>Part C (3rd part)</option>
                                      <option value={4}>Part D (4th part)</option>
                                      <option value={5}>Part E (5th part)</option>
                                    </select>

                                    {/* Available Values Helper for Tag Part */}
                                    {newFilterPart && availableTagParts[`part${newFilterPart}`] && availableTagParts[`part${newFilterPart}`].length > 0 && (
                                      <div className="mt-2">
                                        <div className="text-xs">
                                          <div className="text-zinc-500 dark:text-zinc-400 mb-1">
                                            Available values (click to use):
                                          </div>
                                          <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto">
                                            {availableTagParts[`part${newFilterPart}`].slice(0, showAllFilterValues ? undefined : 20).map((val, idx) => (
                                              <button
                                                key={val.value || idx}
                                              ...
                                              >
                                                {val.value} <span className="text-zinc-400">({val.count})</span>
                                              </button>
                                            ))}
                                            {availableTagParts[`part${newFilterPart}`].length > 20 && (
                                              <button
                                              ...
                                              >
                                                {showAllFilterValues ? '- Show less' : `+ ${availableTagParts[`part${newFilterPart}`].length - 20} more`}
                                              </button>
                                            )}
                                          </div>
                                        </div>
                                            ...
                                            <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto">
                                              {availableHeaderValues.slice(0, showAllFilterValues ? undefined : 20).map((val, idx) => {
                                                ...
                                                return (
                                                  ...
                                                  >
                                                    {displayValue} <span className="text-zinc-400">({displayCount})</span>
                                                  </button>
                                                );
                                              })}
                                              {availableHeaderValues.length > 20 && (
                                                <button
                                                  ...
                                                  className="text-zinc-400 hover:text-brand text-xs px-2 py-0.5 rounded hover:bg-zinc-200/50 dark:hover:bg-zinc-800"
                                                >
                                                  {showAllFilterValues ? '- Show less' : `+ ${availableHeaderValues.length - 20} more`}
                                                </button>
                                              )}
                                            </div>
                                          </div>
                                ...
                                <div className="space-y-2">
                                  <label className="block text-sm font-medium">
                                    {newFilterType === 'value' ? 'Match Value (tag text)' : 'Match Value'}
                                  </label>
                                  <input
                                    type="text"
                                ...
                                <div className="grid grid-cols-2 gap-2 pt-2">
                                  <button
                                    ...
                                    className={cx(
                                      "inline-flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-medium shadow-soft transition-all",
                                      newFilterAction === 'include'
                            ...
                            <div className="pt-2 border-t border-zinc-200/60 dark:border-zinc-800">
                              ...
                              >
                                <i className={cx(
                                  "fa-solid text-xs text-zinc-500",
                                  filterPreviewOpen ? "fa-chevron-down" : "fa-chevron-right",
                                  "transition-transform duration-200"
                                )}></i>
                                <h4 className="text-sm font-semibold">Filter Preview</h4>
                                {filterPreviewTags.length > 0 && (
                                  <span className="text-xs px-2 py-0.5 rounded bg-brand/10 text-brand font-medium ml-auto">
                                    {filterPreviewTags.length} tags
                                  </span>
                                )}
                              </button>
                              ...
                              <div className={cx(
                                "grid transition-all duration-300",
                                filterPreviewOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
                              )}>
                                <div className="overflow-hidden">
                                  <div className="space-y-2">
                                    {filterPreviewLoading ? (
                                      <div className="text-sm text-center py-4 text-zinc-500 dark:text-zinc-400">
                                        <i className="fa-solid fa-spinner fa-spin mr-2"></i>
                                        Loading preview...
                                      </div>
                                    ) : filterPreviewTags.length === 0 ? (
                                      <div className="text-sm text-zinc-500 dark:text-zinc-400 italic text-center py-4">
                                        Click "Preview Tags" to see matching tags
                                      </div>
                                    ) : (
                                      <>
                                        <div className="text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-100/50 dark:bg-zinc-900/50 rounded px-3 py-2 border border-zinc-200/40 dark:border-zinc-800/60">
                                          <i className="fa-solid fa-circle-info mr-1.5"></i>
                                          Showing {filterPreviewTags.length} tag{filterPreviewTags.length !== 1 ? 's' : ''} matching current filters
                                        </div>

                                        <div className={cx(
                                          "flex flex-wrap gap-1.5 p-2 rounded-lg bg-white dark:bg-zinc-900/50 border border-zinc-200/60 dark:border-zinc-800",
                                          !filterPreviewExpanded && "max-h-32 overflow-hidden"
                                        )}>
                                          {filterPreviewTags.map((tag, idx) => (
                                            <span
                                              key={idx}
                    ...
                    <div className="rounded-lg border border-zinc-200/60 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 ml-2">
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <i className={cx(
                            "fa-solid text-sm text-brand",
                            tagMatchingPanelOpen ? "fa-chevron-down" : "fa-chevron-right",
                            "transition-transform duration-200"
                          )}></i>
                          <h3 className="text-sm font-semibold flex items-center">
                          <span className="flex items-center">
                            Custom Tag Matching
                            <HelpIcon content={HELP_CONTENT.customTagMatching} />
                          </span>
                        </h3>
                        </div>
                      </button>
                      ...
                      <div className={cx(
                        "grid transition-all duration-300",
                        tagMatchingPanelOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
                      )}>
                        <div className="overflow-hidden">
                          <div className="px-4 pb-3 space-y-3">
                            ...
                            <div className="space-y-2">
                              <h4 className="text-sm font-semibold">Tag Matching Preset</h4>
                              ...
                              <div className="space-y-2">
                                ...
                                <label className="flex items-start gap-2 cursor-pointer group">
                                  ...
                                  />
                                  <div className="flex-1">
                                    <span className="text-sm font-medium block">Default</span>
                                    <span className="text-xs text-zinc-500 dark:text-zinc-400">
                                      Standard 3-5 part hierarchical tags (e.g., A-B-C, SYS.PUMP.01, TAG-001-A)
                                    </span>
                                  </div>
                                </label>

                                {/* Match Everything Preset */}
                                <label className="flex items-start gap-2 cursor-pointer group">
                                  <input
                                    type="radio"
                                    name="tag-matching-preset"
                                  ...
                                  />
                                  <div className="flex-1">
                                    <span className="text-sm font-medium block">Match Everything</span>
                                    <span className="text-xs text-zinc-500 dark:text-zinc-400">
                                      Accept any text as valid tags (match all alphanumeric sequences)
                                    </span>
                                  </div>
                                </label>
                                ...
                                <label className="flex items-start gap-2 cursor-pointer group">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <span className="text-xs text-zinc-500 dark:text-zinc-400">
                                      Define your own tag matching pattern with custom configuration or regex
                                    </span>
                                  </div>
                                </label>
                              </div>
                              ...
                              <div className="text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-100/50 dark:bg-zinc-900/50 rounded px-3 py-2 border border-zinc-200/40 dark:border-zinc-800/60">
                                <i className="fa-solid fa-circle-info mr-1.5"></i>
                                The preset determines which patterns are recognized as tags in your PDF
                              </div>
                            </div>

                            {/* Custom Configuration Section (only visible when Custom is selected) */}
                            {tagMatchingPreset === 'custom' && (
                              <div className="space-y-3 pt-2 border-t border-zinc-200/60 dark:border-zinc-800">
                                <h4 className="text-sm font-semibold">Custom Configuration</h4>

                                {/* Separators Section */}
                                <div className="space-y-2">
                                  <label className="block text-sm font-medium">Allowed Separators</label>
                                  <div className="flex flex-wrap gap-2">
                                    {availableSeparators.map((sep) => (
                                      ...
                                      >
                                        ...
                                        />
                                        <span className="text-xs font-medium">{sep.label}</span>
                                      </label>
                                    ))}
                                  </div>
                                  <p className="text-xs text-zinc-500 dark:text-zinc-400">
                                    Select which characters can separate tag parts
                                  </p>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                  <div className="space-y-1">
                                    <label htmlFor="min-parts" className="block text-sm font-medium">
                                      Minimum Parts
                                    </label>
                                    <input
                                      id="min-parts"
                                  ...
                                  <div className="space-y-1">
                                    <label htmlFor="max-parts" className="block text-sm font-medium">
                                      Maximum Parts
                                    </label>
                                    <input
                                      id="max-parts"
                                ...
                                <p className="text-xs text-zinc-500 dark:text-zinc-400">
                                  Number of parts in a valid tag (e.g., A-B-C has 3 parts)
                                </p>

                                {/* Part Length Section */}
                                <div className="grid grid-cols-2 gap-3">
                                  <div className="space-y-1">
                                    <label htmlFor="min-part-length" className="block text-sm font-medium">
                                      Min Part Length
                                    </label>
                                    <input
                                      id="min-part-length"
                                  ...
                                  <div className="space-y-1">
                                    <label htmlFor="max-part-length" className="block text-sm font-medium">
                                      Max Part Length
                                    </label>
                                    <input
                                      id="max-part-length"
                                ...
                                <p className="text-xs text-zinc-500 dark:text-zinc-400">
                                  Character length of each part in a tag (e.g., "ABC" has length 3)
                                </p>
                                ...
                                <div className="space-y-2">
                                  <label className="flex items-center gap-2 cursor-pointer">
                                    ...
                                    />
                                    <span className="text-sm font-medium">Allow partial tag matching</span>
                                  </label>
                                  <p className="text-xs text-zinc-500 dark:text-zinc-400 ml-6">
                                    Tags can be matched even if they don't start at word boundaries
                                  </p>
                                </div>
                              </div>
                            )}

                            {/* Custom Regex Section (only visible when Custom preset) */}
                            {tagMatchingPreset === 'custom' && (
                              <div className="space-y-2 pt-2 border-t border-zinc-200/60 dark:border-zinc-800">
                                <div className="flex items-center justify-between">
                                  <h4 className="text-sm font-semibold">Advanced: Custom Regex Pattern</h4>
                                  <a
                                ...
                                <div className="text-xs text-zinc-500 dark:text-zinc-400 bg-amber-50/50 dark:bg-amber-900/10 rounded px-3 py-2 border border-amber-200/40 dark:border-amber-800/60">
                                  <i className="fa-solid fa-triangle-exclamation mr-1.5 text-amber-600 dark:text-amber-500"></i>
                                  If provided, this regex will override the configuration above. Leave empty to use the configuration-based pattern.
                                </div>
                              </div>
                            )}
                            ...
                            <div className="space-y-2 pt-2 border-t border-zinc-200/60 dark:border-zinc-800">
                              ...
                              {!selectedPdfs || selectedPdfs.length === 0 ? (
                                <p className="text-xs text-zinc-500 dark:text-zinc-400 text-center">
                                  Upload a PDF file to preview tag matching
                                </p>
                              ) : null}

                              {/* Preview Results */}
                              {tagMatchingPreviewOpen && !tagMatchingPreviewLoading && (
                                <div className="space-y-2 pt-2">
                                  <div className="flex items-center justify-between p-3 rounded-lg bg-zinc-100/50 dark:bg-zinc-900/50 border border-zinc-200/60 dark:border-zinc-800">
                                    <div className="flex items-center gap-2">
                                      <i className="fa-solid fa-tags text-brand"></i>
                                      <span className="text-sm font-semibold">
                                        {tagMatchingPreviewTags.length} {tagMatchingPreviewTags.length === 1 ? 'tag' : 'tags'} matched
                                      </span>
                                    </div>
                                    {tagMatchingPreviewTags.length > 0 && (
                                      <button
                                        onClick={() => setTagMatchingPreviewExpanded(!tagMatchingPreviewExpanded)}
                                        className="text-xs text-brand hover:underline"
                                      >
                                        {tagMatchingPreviewExpanded ? 'Hide' : 'Show'} Tags
                                      </button>
                                    )}
                                  </div>

                                  {tagMatchingPreviewExpanded && tagMatchingPreviewTags.length > 0 && (
                                    <div className="max-h-48 overflow-y-auto rounded-lg border border-zinc-200/60 dark:border-zinc-800 bg-white dark:bg-zinc-900/50">
                                      <div className="p-3 space-y-1">
                                        {tagMatchingPreviewTags.slice(0, 50).map((tag, index) => (
                                          <div
                                            key={index}
                                        ...
                                        {tagMatchingPreviewTags.length > 50 && (
                                          <div className="text-xs text-zinc-500 dark:text-zinc-400 italic text-center pt-2">
                                            ...and {tagMatchingPreviewTags.length - 50} more tags
                                          </div>
                                        )}
                                      </div>
                                    </div>
                                  ...
                                  {tagMatchingPreviewTags.length === 0 && (
                                    <div className="text-sm text-zinc-500 dark:text-zinc-400 text-center py-4 italic">
                    ...
                    <div className="rounded-lg border border-zinc-200/60 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 ml-2">
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <i className={cx(
                            "fa-solid text-sm text-brand",
                            colorRulesExpanded ? "fa-chevron-down" : "fa-chevron-right",
                            "transition-transform duration-200"
                          )}></i>
                          <h3 className="text-sm font-semibold flex items-center">
                          <span className="flex items-center">
                            Color Rules
                            <HelpIcon content={HELP_CONTENT.colorRules} />
                          </span>
                        </h3>
                        </div>
                        {colorRules.length > 0 && (
                          <span className="text-xs px-2 py-1 rounded bg-brand/10 text-brand font-medium">
                            {colorRules.length} {colorRules.length === 1 ? 'rule' : 'rules'}
                          </span>
                        )}
                      </button>
                      ...
                      <div className={cx(
                        "grid transition-all duration-300",
                        colorRulesExpanded ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
                      )}>
                        <div className="overflow-hidden">
                          <div className="px-4 pb-3 space-y-3">
                            ...
                            <div className="space-y-2">
                              <h4 className="text-sm font-semibold">Default Highlight Color</h4>
                              <label className="flex items-center gap-2 cursor-pointer">
                                <input
                                  type="checkbox"
                                  className="accent-brand h-4 w-4 rounded"
                                ...
                                />
                                <span className="text-sm font-medium">Enable default highlight color</span>
                              </label>
                              {enableDefaultColor && (
                                <div className="ml-6">
                                  <label htmlFor="default-color-picker" className="block text-sm font-medium mb-1">
                                    Default Color
                                  </label>
                                  <input
                                  ...
                                  <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
                                    Used for tags that don't match any color rules
                                  </p>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="space-y-2 pt-2 border-t border-zinc-200/60 dark:border-zinc-800">
                              <h4 className="text-sm font-semibold">Excel Constraint Mode</h4>
                              <label className="flex items-center gap-2 cursor-pointer">
                                <input
                                  type="checkbox"
                                  className="accent-brand h-4 w-4 rounded"
                                ...
                                />
                                <span className="text-sm font-medium">Restrict to Excel column values</span>
                              </label>
                              <div className="text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-100/50 dark:bg-zinc-900/50 rounded px-3 py-2 border border-zinc-200/40 dark:border-zinc-800/60">
                                <i className="fa-solid fa-circle-info mr-1.5"></i>
                                When enabled, only tags found in the Excel file will be highlighted
                              </div>
                              {excelConstraintMode && (
                                  ...
                                  <div className="flex gap-3">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                      <input
                                        ...
                                        className="accent-brand"
                                      />
                                      <span className="text-sm">AND (all rules must match)</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                      <input
                                        type="radio"
                                        ...
                                        className="accent-brand"
                                      />
                                      <span className="text-sm">OR (any rule can match)</span>
                                    </label>
                                  </div>
                                </div>
                              )}
                            ...
                            <div className="space-y-2 pt-2 border-t border-zinc-200/60 dark:border-zinc-800">
                              <div className="flex items-center justify-between">
                                <h4 className="text-sm font-semibold">Active Color Rules</h4>
                                {colorRules.length > 0 && (
                                  <button
                              ...
                              {colorRules.length === 0 ? (
                                <div className="text-sm text-zinc-500 dark:text-zinc-400 italic text-center py-4">
                                  No color rules defined. Click "Add Rule" to create one.
                                </div>
                              ) : (
                                <div className="space-y-2">
                            ...
                            {/* Color Rule Builder Sub-Section */}
                            {colorRuleBuilderOpen && (
                              <div className="space-y-3 pt-2 border-t border-zinc-200/60 dark:border-zinc-800">
                                <div className="flex items-center justify-between">
                                  <h4 className="text-sm font-semibold">Rule Builder</h4>
                                  <button
                                ...
                                {newRuleType === 'tag_part' && (
                                  <div className="space-y-2">
                                    ...
                                    {/* Available Values Helper for Tag Part */}
                                    {newRulePart && availableTagParts[`part${newRulePart}`] && availableTagParts[`part${newRulePart}`].length > 0 && (
                                      <div className="mt-2">
                                        <div className="text-xs">
                                          <div className="text-zinc-500 dark:text-zinc-400 mb-1">
                                            Available values (click to use):
                                          ...
                                          <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto">
                                            {availableTagParts[`part${newRulePart}`].slice(0, showAllRuleValues ? undefined : 20).map((val, idx) => (
                                              <button
                                                key={val.value || idx}
                                            ...
                                            {availableTagParts[`part${newRulePart}`].length > 20 && (
                                              <button
                                                onClick={() => setShowAllRuleValues(!showAllRuleValues)}
                                                className="text-zinc-400 hover:text-brand text-xs px-2 py-0.5 rounded hover:bg-zinc-200/50 dark:hover:bg-zinc-800"
                                              >
                                                {showAllRuleValues ? '- Show less' : `+ ${availableTagParts[`part${newRulePart}`].length - 20} more`}
                                              </button>
                                            )}
                                          </div>
                                        </div>
                                ...
                                {/* Color Picker */}
                                <div className="space-y-2">
                                  <label htmlFor="new-rule-color" className="block text-sm font-medium">
                                    Highlight Color
                                  </label>
                                  <input
                                    id="new-rule-color"
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                >
                  ...
                  <span className="flex items-center gap-1.5">
                    Start
                    <HelpIcon content={HELP_CONTENT.startProcessing} position="top" />
                  </span>
                </button>
                <button
                ...
                >
                  ...
                  <span className="flex items-center gap-1.5">
                    Test Run (100 tags)
                    <HelpIcon content={HELP_CONTENT.testRun} position="top" />
                  </span>
                </button>
                <button
          ...
          {/* Toast */}
          {toast && (
            <div className="fixed bottom-6 left-0 right-0 mx-auto w-[92%] max-w-md z-50">
              <div
                className={cx(
                  "rounded-xl px-4 py-3 shadow-soft border backdrop-blur",
                  "bg-white/90 dark:bg-zinc-900/90 border-zinc-200/60 dark:border-zinc-800",
                  "flex items-center gap-3"
                )}
              >
                <i
                  className={cx(
                    "fa-solid",
                    toast.type === "error" && "fa-circle-xmark text-danger",
                    toast.type === "success" && "fa-circle-check text-success",
                    toast.type === "warning" && "fa-triangle-exclamation text-warning",
                    toast.type === "info" && "fa-circle-info text-brand"
                  )}
                />
          ...
          {selectedPdfs.length > 0 && selectedExcel && (
            <button
          ...
          {/* Preview Modal */}
          {previewModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={closePreviewModal}>
              <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-2xl w-full h-full max-w-[90vw] max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                {/* Modal Header */}
                <div className="flex-shrink-0 border-b border-zinc-200 dark:border-zinc-700 p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      title="Close (ESC)"
                    >
                      <i className="fa-solid fa-times text-xl text-zinc-600 dark:text-zinc-400"></i>
                    </button>
                  </div>

                  {/* Warning Message */}
                  <div className="mb-3 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-lg flex items-start gap-2">
                    <i className="fa-solid fa-info-circle text-amber-600 dark:text-amber-400 mt-0.5"></i>
                    <p className="text-sm text-amber-800 dark:text-amber-200">
                      <strong>Note:</strong> Comment popups are not visible in this preview. Download the preview to see all comment annotations.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-zinc-500 dark:text-zinc-400">
                          of {previewModalData?.total_pages || '?'}
                        </span>
                      </div>
                    ...
                    <div className="flex items-center gap-2 ml-auto">
                      ...
                      <span className="text-sm font-medium text-zinc-600 dark:text-zinc-400 min-w-[3.5rem] text-center">
                  ...
                  {previewModalData && (
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-4">
                      <div className="flex flex-wrap items-center gap-4 text-sm">
                        {/* Tag Statistics Badge */}
                        {previewModalData.stats && (
                          <div className="flex items-center gap-3 px-3 py-2 bg-gradient-to-r from-blue-50 to-green-50 dark:from-blue-900/20 dark:to-green-900/20 border border-blue-200 dark:border-blue-800/50 rounded-lg">
                            <div className="flex items-center gap-2">
                              <i className="fa-solid fa-tags text-blue-600 dark:text-blue-400"></i>
                            ...
                            <div className="flex items-center gap-1">
                              <span className="text-lg font-bold text-green-600 dark:text-green-400">
                                {previewModalData.stats.colored_tags}
                              </span>
                              <span className="text-zinc-500 dark:text-zinc-400">/</span>
                              <span className="text-lg font-bold text-blue-600 dark:text-blue-400">
                                {previewModalData.stats.total_tags}
                              </span>
                              <span className="text-xs text-zinc-500 dark:text-zinc-400 ml-1">
                                colored
                              </span>
                            </div>
                            {previewModalData.stats.total_tags > 0 && (
                              <div className="flex items-center gap-1 px-2 py-0.5 bg-white/50 dark:bg-zinc-800/50 rounded-full">
                                <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400">
                                  {Math.round((previewModalData.stats.colored_tags / previewModalData.stats.total_tags) * 100)}%
                                </span>
                              </div>
                            )}
                          </div>
                        )}
                        {previewModalData.stats.conflict_count > 0 && (
                          <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400">
                            <i className="fa-solid fa-exclamation-triangle"></i>
                            <span>{previewModalData.stats.conflict_count} conflicts</span>
                          </div>
                        )}
                      </div>
                      {previewModalData.download_url && (
                        <button
                          onClick={() => {
                            window.location.href = previewModalData.download_url;
                            setToast({ type: "success", text: `Downloading ${previewModalData.preview_clean_name}` });
                          }}
                        ...
                        >
                          <i className="fa-solid fa-download"></i>
                          <span className="hidden sm:inline">Download Preview PDF</span>
                  ...
                  {previewModalLoading ? (
                    <div className="flex items-center justify-center h-full">
                      <div className="text-center">
                        <i className="fa-solid fa-spinner fa-spin text-4xl text-brand mb-3"></i>
                    ...
                    <div className="flex items-center justify-center h-full">
                      <p className="text-zinc-500 dark:text-zinc-400">No preview data</p>
                    </div>
                  )}
                </div>

                {/* Modal Footer */}
                {previewModalData && previewModalData.legend && (
                  <div className="flex-shrink-0 border-t border-zinc-200 dark:border-zinc-700 p-4 max-h-32 overflow-y-auto">
                    <div className="flex flex-wrap items-center gap-3">
                      <span className="text-sm font-medium text-zinc-600 dark:text-zinc-400">Legend:</span>
                      {(Array.isArray(previewModalData.legend) ? previewModalData.legend : Object.entries(previewModalData.legend)).map((item, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex items-center gap-2">
                            ...
                            ></div>
                            <span className="text-sm text-zinc-700 dark:text-zinc-300">{tagText}</span>
                          </div>
                        );
                      })}
                    </div>
